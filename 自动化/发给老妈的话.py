...
print(get_iciba_everyday_chicken_soup("鹏鹏"))
...
xingming='大鹏'
text=get_iciba_everyday_chicken_soup(xingming)+geturltext(xingming,city_html)

def writeToFile():
    with open("鹏鹏の问候.txt", 'a', encoding='utf-8') as f:  # 将句子写入文件，调整编码为'utf-8',添加写模式
        f.write(text + '\n')  # 在每一句内容后加上换行符
        f.write('-----------------------------------------------------------------------------'+'\n')
